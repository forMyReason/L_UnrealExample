	...
	[
		...
		[
			...
			[
				...
				.Visibility(this, &SPivotPreset::GetOptionsTextVisibility)
...
void SPivotPreset::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	if (WidthLastFrame != AllottedGeometry.Size.X)
	{
		...
		TextUIWrapBox->SetWrapSize(WidthLastFrame);
		PresetOptionsWrapBox->SetWrapSize(WidthLastFrame);
		PresetWrapBox->SetWrapSize(WidthLastFrame);
		PresetSubWrapBox1->SetWrapSize(WidthLastFrame);
		PresetSubWrapBox2->SetWrapSize(WidthLastFrame);
	}
...
EVisibility SPivotPreset::GetOptionsTextVisibility() const
{
	return bOptionsDisplayed ? EVisibility::Collapsed : EVisibility::Visible;
}
			...
			if (InPresetType == EPivotToolPresetType::PresetType_Edge)
			{
				...
				Canvas->AddSlot()
					...
					[
						...
						.ToolTipText(this, &SPivotPreset::GetPivotPresetTooltip, ReOrderedPreset)
