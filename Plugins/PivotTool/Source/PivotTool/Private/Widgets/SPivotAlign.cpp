...
void SPivotAlign::Construct(const FArguments& InArgs)
{
	...
	struct Local
	{
		static void AddButton(TSharedPtr<SHorizontalBox> HorizontalBox, const FText& ButtonName, const FName& ButtonStyle, const TSharedPtr<FUICommandInfo>& InUICommandInfo, const FOnClicked& OnClick, float ButtonContentPadding = 2.f, float ButtonTextMinDesiredWidth = 15.f)
		{
			...
			[
				...
				.OnClicked(OnClick)
				.ToolTip(FPivotToolCommands::Get().CreateToolTip(InUICommandInfo))
				.VAlign(VAlign_Center)
				[
					SNew(STextBlock)
	...
	ButtonBox1->AddSlot().AutoWidth().Padding(4, 2, 4, 2).HAlign(HAlign_Center).VAlign(VAlign_Center)
	[
		SNew(STextBlock)
	...
	Local::AddButton(ButtonBox1,
		LOCTEXT("All", "All"),
		DefautlButtonStyle,
		FPivotToolCommands::Get().AlignActorsAll,
	...
	Local::AddButton(ButtonBox2,
		LOCTEXT("X", "X"),
		ColorXButtonStyle,
		FPivotToolCommands::Get().AlignActorsX,
	...
	Local::AddButton(ButtonBox2,
		LOCTEXT("-X", "-X"),
		ColorXButtonStyle,
		FPivotToolCommands::Get().AlignActorsNegX,
	...
	Local::AddButton(ButtonBox3,
		LOCTEXT("Y", "Y"),
		ColorYButtonStyle,
		FPivotToolCommands::Get().AlignActorsY,
	...
	Local::AddButton(ButtonBox3,
		LOCTEXT("-Y", "-Y"),
		ColorYButtonStyle,
		FPivotToolCommands::Get().AlignActorsNegY,
	...
	Local::AddButton(ButtonBox4,
		LOCTEXT("Z", "Z"),
		ColorZButtonStyle,
		FPivotToolCommands::Get().AlignActorsZ,
	...
	Local::AddButton(ButtonBox4,
		LOCTEXT("-Z", "-Z"),
		ColorZButtonStyle,
		FPivotToolCommands::Get().AlignActorsNegZ,
...
{
	AlignBox->SetWrapSize(InWrapWidth);
